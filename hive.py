...
class Hive(Actor):
    ...
    def register_actor(self, actor):
        ...
        elif actor.local_id == self.hive_id and actor != self:
...
#         if message.directive != "self_reply":
#             pdb.set_trace()
        # Route appropriately here
        actor_id, hive_id = split_id(message.to)
        if hive_id == None and actor_id != None:
    ...
    def return_to_sender(self, message, directive="error.no_such_actor"):
        ...
        if message.wants_reply and message.from_id != message.to:
            self.send_message(
                ...
                from_id=self.hive_id, in_reply_to=message.id)
